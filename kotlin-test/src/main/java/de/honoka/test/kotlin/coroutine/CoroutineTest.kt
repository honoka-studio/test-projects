...
object CoroutineTest {
	...
	private suspend fun fun1() {
		for(i in 1..30) {
			ColorfulText.of().red(
				"[${Thread.currentThread().name}] [${System.currentTimeMillis()}] fun1: $i"
			).println()
			/*
	...
	private suspend fun fun2() {
		for(i in 1..30) {
			ColorfulText.of().green(
				"[${Thread.currentThread().name}] [${System.currentTimeMillis()}] fun2: $i"
			).println()
			if(i % 10 == 0) delay(1)
		}
	}
	...
	private suspend fun fun3() {
		for(i in 1..30) {
			ColorfulText.of().blue(
				"[${Thread.currentThread().name}] [${System.currentTimeMillis()}] fun3: $i"
			).println()
			if(i % 10 == 0) delay(1)
		}
	}
